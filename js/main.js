...
export function main() {
  ...
  // No solid background — gradient sky dome covers everything
  ...
  starGeometry.setAttribute('position', new THREE.BufferAttribute(_starPositions, 3));
  starGeometry.setAttribute('aPhase', new THREE.BufferAttribute(_starPhases, 1));
  starGeometry.setAttribute('aBrightness', new THREE.BufferAttribute(_starBrightness, 1));
  ...
  const skyColorDayHorizon   = new THREE.Color(DAY_NIGHT.skyDayHorizonColor   || 0xB8D4FF);
  const skyColorNightHorizon = new THREE.Color(DAY_NIGHT.skyNightHorizonColor || 0x060D1A);
  const skyColorDawnHorizon  = new THREE.Color(DAY_NIGHT.skyDawnHorizonColor  || 0xFF8C2A);
  const skyColorDuskHorizon  = new THREE.Color(DAY_NIGHT.skyDuskHorizonColor  || 0xFF4D10);
  const currentSkyZenith  = new THREE.Color(DAY_NIGHT.skyDayColor);
  const currentSkyHorizon = new THREE.Color(DAY_NIGHT.skyDayHorizonColor || 0xB8D4FF);

  // === SKY DOME — gradient sphere (horizon vs zenith) ===
  ...
  const moonPhasesTexture = (cm.materials && cm.materials.moonPhases && cm.materials.moonPhases.map) ? cm.materials.moonPhases.map.clone() : null;
  if (moonPhasesTexture) {
    moonPhasesTexture.needsUpdate = true;
  ...
  const moonMesh = new THREE.Mesh(new THREE.PlaneGeometry(DAY_NIGHT.moonSize, DAY_NIGHT.moonSize), moonMaterial);
  ...
  const playerMaterial = new THREE.MeshStandardMaterial({ transparent: true, opacity: 0, alphaTest: 0.1});
  ...
  const player_material = new THREE.MeshStandardMaterial({ map: cm.materials.playerskin.map, transparent: true });
  ...
  function uvmapper(geom, faceRects, cols = 16, rows = 16) {
    ...
    for(let f=0; f<6; f++){
        ...
        for(let i=0;i<6;i++){
            ...
            uvs[j+1] = 1-(vMin + (1-v)*(vMax-vMin)); // flip Y
        }
    }
  ...
  function createPart(size, faceImages) {
    const geom = new THREE.BoxGeometry(...size).toNonIndexed();
    uvmapper(geom, faceImages);
    return new THREE.Mesh(geom, player_material);
  }

  // create player parts [xlength, ylength, zlength], faceImages: [right, left, top, bottom, back, front] (0-63 index into skin)
  const player_head = createPart([0.5, 0.5, 0.5], [[32,49],[36,53],[2,19],[4,21],[38,55],[34,51]]);
  const player_body = createPart([0.5, 0.75, 0.25], [[84,116],[87,119],[69,70],[71,72],[88,121],[85,118]]);
  const player_leftArm = createPart([0.25, 0.75, 0.25], [[216,248],[218,250],[201,201],[202,202],[219,250],[217,249]]);
  const player_rightArm = createPart([0.25, 0.75, 0.25], [[90,122],[92,124],[75,75],[76,76],[93,125],[91,123]]);
  const player_leftLeg = createPart([0.25, 0.75, 0.25], [[212,244],[214,246],[197,197],[198,198],[215,247],[213,245]]);
  const player_rightLeg = createPart([0.25, 0.75, 0.25], [[80,112],[82,114],[65,65],[66,66],[83,115],[81,113]]);
  ...
  const move = { forward: false, backward: false, left: false, right: false, sprint: false, crouch: false, up: false, down: false };
  ...
  function onMouseMove(e) {
    ...
    if (isSpectator) {
      ...
      spectatorPitch = Math.max(-PI_2 + 0.01, Math.min(PI_2 - 0.01, spectatorPitch));
  ...
  function animate() {
    ...
    if (typeof player_leftArmPivot !== 'undefined') player_leftArmPivot.rotation.x = Math.sin(walkTimer * walkFreq) * armAmp * speedFactor;
    if (typeof player_rightArmPivot !== 'undefined') player_rightArmPivot.rotation.x = Math.sin(walkTimer * walkFreq + Math.PI) * armAmp * speedFactor;
    if (typeof player_leftLegPivot !== 'undefined') player_leftLegPivot.rotation.x = Math.sin(walkTimer * walkFreq + Math.PI) * legAmp * speedFactor;
    if (typeof player_rightLegPivot !== 'undefined') player_rightLegPivot.rotation.x = Math.sin(walkTimer * walkFreq) * legAmp * speedFactor;
    ...
    try {
      if (sunBackdropMesh) {
        ...
        const backdropOpacity = Math.pow(Math.max(0, 1 - _backdropSunH / 0.25), 2);
        sunBackdropMesh.material.opacity = backdropOpacity;
      }
    } catch (err) { console.warn('sun backdrop update failed', err); }

    const moonAngle = angle + Math.PI;
    celestialPos.set(Math.cos(moonAngle) * sunDist + player.position.x, Math.sin(moonAngle) * sunDist, Math.sin(moonAngle * 0.5) + player.position.z);
    ...
    if (currentCycleIndex !== lastMoonCycleIndex) {
      ...
      currentMoonPhase = ((currentCycleIndex % MOON_PHASE_COUNT) + MOON_PHASE_COUNT) % MOON_PHASE_COUNT;
      if (moonPhasesTexture) {
        const col = currentMoonPhase % MOON_COLS;
        const row = Math.floor(currentMoonPhase / MOON_COLS);
        moonPhasesTexture.offset.set(col / MOON_COLS, (MOON_ROWS - 1 - row) / MOON_ROWS);
      }
    }
    ...
    const transitionFactor = Math.max(0, 1 - Math.abs(sunHeight) / 0.3) * (Math.abs(sunHeight) < 0.3 ? 1 : 0);
    ...
    starField.position.set(player.position.x, player.position.y, player.position.z);
    skyDome.position.set(player.position.x, player.position.y, player.position.z);
    starField.rotation.y = (t / CYCLE_LENGTH) * Math.PI * 2;
    starField.rotation.x = 0.15;
    ...
    if (typeof clouds !== 'undefined' && clouds && clouds.group) {
      ...
      if (clouds.materials) {
        _cloudTint.copy(_cloudDayColor).lerp(_cloudNightColor, 1 - Math.min(1, ambientRatio / 2.0));
        ...
        clouds.materials.forEach((mat, idx) => {
          ...
          else if (idx === 1) mat.color.copy(_cloudTint).multiplyScalar(sideRatio);
    ...
    try { if (typeof blockBreaker !== 'undefined' && blockBreaker) blockBreaker.update(gameDelta); } catch (e) {}
